...
This script natively connects to `localhost:8080`, executes the requested interactions exactly as outlined above, and leverages Playwright's built-in `recordVideo` functionality to reliably export a `.webm` file into the `videos/` directory.

### ⚠️ Prerequisites Before Running Any Script

> These must be satisfied **before** running `npm run build-video` or `npm run record`.

**1. Start the backend with the custom script:**
```bash
# From the dev-video-automation root
...
1. **Generación (TTS)**: `generate_audio.js` crea los archivos `.mp3` utilizando textos acortados para lograr una narrativa fluida.
2. **Mezcla Inteligente**: `merge_video_audio.js` utiliza **FFmpeg** para:
...
Local build artifacts remain under `videos/` and are **gitignored**. Optional: attach `final_showcase_with_audio.webm` to a GitHub Release for direct download.
